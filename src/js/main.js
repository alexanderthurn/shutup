import {getCursorPosition,
    ...
    setAudioContextWithFallback} from './helper.js'
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    var lineVolumeInner = document.getElementById('lineVolumeInner')
    ...
    function updateYAxisDescription() {

        if(audioCtx.state === 'suspended' || audioCtxMic.state === 'suspended' || oscillator.context.state === 'suspended') {
            lineVolumeInner.innerText = INTL_Y_AXIS_LEVEL_DESCRIPTION_BEFORE_START
        } else {
            lineVolumeInner.innerText = INTL_Y_AXIS_LEVEL_DESCRIPTION
        }
    ...
    javascriptNode.onaudioprocess = function () {
        ...
        var average = getAverageValue(array)
    ...
    canvasHistory.addEventListener('click', function (event) {
        ...
        if(audioCtx.state === 'suspended') {
            audioCtx.resume().then(updateYAxisDescription)
        }

        if(audioCtxMic.state === 'suspended') {
            audioCtxMic.resume().then(updateYAxisDescription)
        }

        if (oscillator.context.state === 'suspended') {
            oscillator.context.resume().then(updateYAxisDescription)
        }
    ...
    var updateCanvasRegular = function () {
        ...
        var currentValue = globalAverage // getAverageValue(values.slice(0, 10))
